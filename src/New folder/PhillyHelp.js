...
const T = {
  en: {
    ...
    noOpenResources:"No resources open right now — call Philly 311 (dial 211) for immediate help.",
  ...
  es: {
    appName:"PhillyAyuda", tagline:"Encuentra ayuda cerca de ti, ahora mismo.", county:"Condado de Delaware, PA", zip:"Philadelphia · 19121",
...
const ZIP_COORDS = {
  "19121":{ lat:39.8926, lng:-75.3693 }, // Philadelphia
    ...
    <div className="dfi">
      <div style={{padding:"16px 24px 12px"}}>
        ...
        <div style={{background:"#F0F9F4",borderRadius:14,padding:12,marginBottom:10,border:"1px solid rgba(0,53,148,0.15)"}}>
          ...
          {zip.length===5 && ZIP_COORDS[zip] && (
            <div style={{fontSize:11,color:"#003594",marginTop:6,fontWeight:600}}>Showing resources within 10 miles of {zip}</div>
          )}
        </div>
      ...
      <div style={{padding:"12px 24px"}}>
        ...
        {results.length===0&&(
          <div style={{textAlign:"center",padding:"32px 0"}}>
            ...
            <div style={{fontSize:12,color:"#6B7C6E",lineHeight:1.6}}>Try a nearby zip code, or call Philly 311 (dial 211) for help finding resources anywhere in Philadelphia.</div>
          </div>
        )}
        {/* 211 upgrade note */}
...
function VolunteerScreen({ lang }) {
  const t=T[lang]||T.en;
  const opps=[
    {org:"Lifewerks Food Pantry",role:"Pantry Volunteer",time:"Tuesdays 5:30–8:30 PM",icon:"🍽",color:"#003594"},
    {org:"DIFAN Network",role:"Food Distributor",time:"Tuesdays & Fridays",icon:"📦",color:"#0046AD"},
    {org:"Media Food Bank",role:"Donation Sorter",time:"Thursdays 5–8 PM",icon:"🏪️",color:"#002060"},
    {org:"Philly Helping Hands",role:"Driver / Delivery",time:"Flexible scheduling",icon:"🚗",color:"#F4A261"},
  ...
  return (
    <div className="dfi">
      <div style={{padding:"16px 24px 0"}}>
        ...
        {[{key:"name",label:t.orgName,placeholder:"e.g. Philadelphia Community Pantry"},{key:"address",label:t.orgAddress,placeholder:"123 Main St, Philadelphia PA 19121"},{key:"phone",label:t.orgPhone,placeholder:"610-555-0000"},{key:"hours",label:t.orgHours,placeholder:"e.g. Tuesdays 5–7 PM, Saturdays 10 AM–12 PM"}].map(f=>(
...
Key local resources:
- Lifewerks Food Pantry: 25 Cedar Rd, Philadelphia — Tuesdays 6–8 PM, choice pantry, 610-872-3344
- DIFAN Philadelphia: 25 Cedar Rd — Tues 6:30–8 PM, Fri 4–6 PM, 484-326-5362
- Media Food Bank: 350 W State St Media — Thurs 6–8 PM, Sun 1–2 PM, 610-566-3172
- Philly Helping Hands: diapers, pet supplies, referrals, 484-474-0590
- Legal Aid of SE PA: free legal help, 877-429-5994
- Philly 311: dial 211 for any social service
...
Keep responses short, warm, and actionable. Always give a phone number when recommending a resource. If someone seems in crisis, lead with 988 or 911.`;
      ...
      const reply=data.content?.[0]?.text||"I'm sorry, I couldn't find an answer. Please call Philly 311 (dial 211) for immediate help.";
      setMessages(m=>[...m,{role:"ai",text:reply}]);
    } catch(e) {
      setMessages(m=>[...m,{role:"ai",text:"I'm having trouble connecting. Please call Philly 311 (dial 211) for immediate help finding resources."}]);
    }
    setLoading(false);
  }
...
function DonateModal({ onClose, lang }) {
  ...
  const orgs=[
    {name:"Lifewerks Food Pantry",icon:"🍽",desc:"Choice pantry · Philadelphia"},
    {name:"DIFAN Philadelphia",icon:"📦",desc:"Interfaith food network"},
    {name:"Media Food Bank",icon:"🏪",desc:"Media, PA · Thurs & Sun"},
    {name:"Philly Helping Hands",icon:"🤝",desc:"Diapers, supplies, referrals"},
...
function NotifOverlay({ onClose, lang }) {
  const t=T[lang]||T.en;
  const notifs=[
    {icon:"🍽",bg:"#003594",title:"Lifewerks is open now!",body:"Food pantry open until 8:00 PM tonight · 0.3 mi away",time:"now"},
