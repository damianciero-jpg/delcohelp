...
const HOTLINES = [
  ...
  { id:5, name:"Delaware County Crisis", sub:"Mental health emergency line", number:"610-565-4300", color:"#023E8A", bg:"#F0F4FF", icon:"🧩" },
  ...
  { id:9, name:"Homeless Shelter Hotline", sub:"Delaware County shelter line", number:"610-713-2000", color:"#F4A261", bg:"#FFF8F0", icon:"🏠️" },
...
function getImpactStats() {
  try {
    ...
    return [
      { label:"Resources Viewed", value: count("resource_viewed"), icon:"🔍", color:"#40916C" },
      { label:"Benefits Checked", value: count("eligibility_quiz_opened"), icon:"📋", color:"#2D6A4F" },
  ...
  return (
    <div className="dfi">
      <div style={{background:"linear-gradient(160deg,#2D6A4F 0%,#40916C 100%)",padding:"16px 24px 24px",borderRadius:"0 0 28px 28px",marginBottom:16}}>
        ...
        <div style={{fontFamily:"'DM Serif Display',serif",fontSize:24,color:"white",lineHeight:1.25,marginBottom:16}}>{t.tagline}</div>
...
function VolunteerScreen({ lang }) {
  const t=T[lang]||T.en;
  const opps=[
    {org:"Lifewerks Food Pantry",role:"Pantry Volunteer",time:"Tuesdays 5:30–8:30 PM",icon:"🍽",color:"#2D6A4F",phone:"6108723344"},
    {org:"DIFAN Network",role:"Food Distributor",time:"Tuesdays & Fridays",icon:"📦",color:"#40916C",phone:"4843265362"},
    {org:"Media Food Bank",role:"Donation Sorter",time:"Thursdays 5–8 PM",icon:"🏪️",color:"#1B4332",phone:"6105663172"},
    ...
    <div className="dfi">
      ...
      <div style={{padding:"0 24px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          {stats.map((s,i)=>(
            <div key={i} className="impact-stat">
              ...
              <div style={{fontSize:11,fontWeight:600,color:"#1C2B1E",marginTop:4}}>{s.label}</div>
            </div>
          ))}
        </div>
        <div style={{background:"#F0F9F4",borderRadius:16,padding:16,marginBottom:16,border:"1px solid rgba(45,106,79,0.15)"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#1B4332",marginBottom:6}}>About DelcoHelp</div>
        ...
        <div style={{background:"white",borderRadius:16,padding:16,marginBottom:20,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
          <div style={{fontSize:12,fontWeight:700,color:"#6B7C6E",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>Questions or feedback?</div>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{fontFamily:"'DM Serif Display',serif",fontSize:20,color:"#1C2B1E",marginBottom:4}}>Support Local Pantries</div>
        <div style={{fontSize:13,color:"#6B7C6E",marginBottom:16}}>Call the pantry directly — 100% of your donation reaches them</div>
        ...
        <div style={{fontSize:11,color:"#6B7C6E",textAlign:"center",lineHeight:1.5}}>Online donation processing coming soon. Call during open hours to give by card or check.</div>
      </div>
    </div>
  );
}
...
function NotifOverlay({ onClose, lang }) {
  const t=T[lang]||T.en;
  const notifs=[
    {icon:"🍽",bg:"#2D6A4F",title:"Lifewerks Food Pantry — Tuesdays 6–8 PM",body:"Choice pantry at 25 Cedar Rd, Wallingford · No appointment needed",time:"weekly"},
    {icon:"📋",bg:"#40916C",title:"Check your benefits eligibility",body:"SNAP, WIC, LIHEAP and Medicaid — takes 60 seconds in the Benefits tab",time:"tip"},
...
function AuthModal({ onClose, user, onSignIn, onSignOut }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{textAlign:"center",padding:"8px 0 20px"}}>
          <div style={{width:60,height:60,borderRadius:"50%",background:"#2D6A4F18",margin:"0 auto 12px",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
            {user.photoURL
        ...
        <div style={{background:"#F0F9F4",borderRadius:14,padding:14,marginBottom:16,border:"1px solid rgba(45,106,79,0.15)"}}>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={e=>e.stopPropagation()}>
        <div className="modal-handle"/>
        <div style={{fontFamily:"'DM Serif Display',serif",fontSize:20,color:"#1C2B1E",marginBottom:4}}>Sign In — Optional</div>
        ...
        <div style={{background:"#F0F9F4",borderRadius:14,padding:14,marginBottom:16,border:"1px solid rgba(45,106,79,0.15)"}}>
          <div style={{fontSize:12,fontWeight:700,color:"#2D6A4F",marginBottom:8,textTransform:"uppercase",letterSpacing:"0.06em"}}>What syncs to your account</div>
      ...
      <div className="dh">
        <div className="dh-sb">
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            {FIREBASE_ENABLED&&(
              <div onClick={()=>setShowAuth(true)} title={user?"Account":"Sign in"} style={{cursor:"pointer",width:22,height:22,borderRadius:"50%",overflow:"hidden",background:user?"#2D6A4F":"rgba(0,0,0,0.08)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                {user?.photoURL
                  ? <img src={user.photoURL} style={{width:22,height:22}} alt="" referrerPolicy="no-referrer"/>
                  : <span style={{fontSize:user?11:13,color:user?"white":"#6B7C6E",fontWeight:700,lineHeight:1}}>{user?user.displayName?.[0]||"U":"👤"}</span>}
              </div>
            )}
          </div>
        </div>
